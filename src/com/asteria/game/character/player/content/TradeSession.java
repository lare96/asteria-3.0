...
public final class TradeSession {
     ...
     */
    private final ItemContainer container = new ItemContainer(28,
            ItemContainerPolicy.NORMAL);
    ...
    public void add(Item item, int inventorySlot) {
        ...
        if (!Item.valid(item) || !player.getInventory().contains(item.getId()))
            return;
        if (Arrays.stream(Settings.ITEM_UNTRADEABLE).anyMatch(i -> i == item
                .getId())) {
        ...
        else if (item.getAmount() > player.getInventory().get(inventorySlot)
        ...
        if (container.add(item)) {
            ...
            player.getEncoder().sendItemsOnInterface(3322, player
                    .getInventory().container());
            int length = container.size();
            player.getEncoder().sendItemsOnInterface(3415, container
                    .container(), length);
            other.getEncoder().sendItemsOnInterface(3416, container.container
                    (), length);
    ...
    public void request(Player requester) {
        if (requester.getTradeSession().inTradeSession()) {
            requester.getEncoder().sendMessage("You are already in a trade " +
                    "session!");
            return;
        }
        if (inTradeSession()) {
            requester.getEncoder().sendMessage("They are already in a trade " +
                    "session!");
            return;
        }
        if (player.equals(requester)) {
            requester.getEncoder().sendMessage("You cannot initiate a trade " +
        ...
        requester.getEncoder().sendMessage(player.getFormatUsername() +
                ":tradereq:");
    }
    ...
    public void execute(TradeStage stage) {
        ...
        switch (stage) {
            case OFFER:
                ...
                player.getEncoder().sendString("", 3431);
                player.getEncoder().sendString("Are you sure you want to make" +
                        " this trade?", 3535);
                player.getEncoder().sendInventoryInterface(3323, 3321);
                break;
            case FIRST_ACCEPT:
                player.getEncoder().sendItemsOnInterface(3214, player
                        .getInventory().container());
                player.getEncoder().sendString(getItemNames(container
                        .container()), 3557);
                player.getEncoder().sendString(getItemNames(other
                        .getTradeSession().getContainer().container()), 3558);
    ...
    public void reset(boolean declined) {
        ...
        if (declined) {
            other.getEncoder().sendMessage("The other player has declined the" +
                    " trade!");
    ...
    public boolean inTradeSession() {
        return stage == TradeStage.OFFER || stage == TradeStage.FIRST_ACCEPT
    ...
    private String name(Player player) {
        return player.getFormatUsername().concat(player.getRights().equal
                (Rights.MODERATOR) ? "@cr1@" : player.getRights().greater
                (Rights.MODERATOR) ? "@cr2@" : "");
    }
    ...
    private String getItemNames(Item[] items) {
        ...
        for (Item item : items) {
            ...
            tradeAmount = item.getAmount() >= 1000 && item.getAmount() <
                    1000000 ? "@cya@" + (item.getAmount() / 1000) + "K @whi@" +
                    "(" + item.getAmount() + ")" : item.getAmount() >=
                    1000000 ? "@gre@" + (item.getAmount() / 1000000) + " " +
                    "million @whi@(" + item.getAmount() + ")" : "" + item
                    .getAmount();
            tradeItems = count == 0 ? item.getDefinition().getName() :
