...
public final class PlayerDeath extends CharacterDeath<Player> {
    ...
    private void calculateDropItems(Player character, Optional<Player> killer) {
        ...
        Arrays.stream(Settings.KEEP_ON_DEATH).filter(
            id -> character.getEquipment().unequipItem(new Item(id), false) || character.getInventory().remove(new Item(id)))
            .forEach(id -> keep.add(new Item(id)));
        ...
        items.stream().forEach(
            item -> ItemNodeManager.register(!killer.isPresent() ? new ItemNodeStatic(item, character.getPosition())
