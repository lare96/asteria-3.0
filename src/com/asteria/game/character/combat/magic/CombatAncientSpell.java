    ...
    public void executeOnHit(CharacterNode cast, CharacterNode castOn, boolean accurate, int damage) {
        if (accurate) {
            ...
            while (it.hasNext()) {
                ...
                if (character == null || !character.getPosition().withinDistance(castOn.getPosition(), radius()) || character
                    .equals(cast) || character.equals(castOn) || character.getCurrentHealth() <= 0 || character.isDead()) {
                    continue;
                }
