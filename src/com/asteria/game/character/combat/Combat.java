...
public final class Combat {
    ...
    public static boolean isAccurate(CharacterNode attacker, CharacterNode victim, CombatType type) {
        ...
        if (attacker.getType() == NodeType.PLAYER) {
            ...
            equipmentBonus = type == CombatType.MAGIC ? player.getBonus()[Combat.ATTACK_MAGIC] : player.getBonus()[player
        ...
        if (victim.getType() == NodeType.PLAYER) {
            ...
            if (bonusType == -1) {
                equipmentBonus = type == CombatType.MAGIC ? player.getBonus()[Combat.DEFENCE_MAGIC]
    ...
    public static boolean checkAttackDistance(CombatBuilder builder) {
        ...
        if (!movement.isMovementDone() && !otherMovement.isMovementDone() && !movement.isLockMovement() && !builder
            .getCharacter().isFrozen()) {
            distance += 1;
