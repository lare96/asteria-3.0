...
public final class CombatSessionAttack extends Task {
    ...
    public CombatSessionAttack(CombatBuilder builder, CombatSessionData data) {
        super(Combat.getDelay(data.getType()), data.getType() == CombatType
                .MELEE);
    ...
    public void execute() {
        ...
        if (attacker.isDead() || !attacker.isRegistered() || victim.isDead()
                || !victim.isRegistered()) {
            this.cancel();
            return;
        }
        ...
        if (data.getHits().length != 0 && data.getType() != CombatType.MAGIC
                || data.isAccurate()) {
        ...
        if (!data.isAccurate()) {
            if (data.getType() == CombatType.MAGIC) {
                ...
                attacker.getCurrentlyCasting().executeOnHit(attacker, victim,
                        false, 0);
            ...
            else if (data.getType() == CombatType.RANGED && attacker.getType
                    () == NodeType.PLAYER && random.nextBoolean()) {
                Player player = (Player) attacker;
                if (player.getFireAmmo() > 0) {
                    ItemNodeManager.register(new ItemNode(new Item(player
                            .getFireAmmo()), victim.getPosition(), player),
                            true);
        ...
        if (victim.getType() == NodeType.PLAYER) {
            ...
            victim.animation(new Animation(((Npc) victim).getDefinition()
                    .getDefenceAnimation(), AnimationPriority.LOW));
        }
        ...
        if (random.nextInt(4) == 0) {
            ...
            if (builder.getVictim().getType() == NodeType.PLAYER) {
                ...
                if (Combat.isFullTorags(builder.getCharacter())) {
                    victim.getRunEnergy().decrementAndGet(random.inclusive(1,
                            100));
                    victim.graphic(new Graphic(399));
                }
                else if (Combat.isFullAhrims(builder.getCharacter()) &&
                        victim.getSkills()[Skills.STRENGTH].getLevel() >=
                                victim.getSkills()[Skills.STRENGTH]
                                        .getRealLevel()) {
                    victim.getSkills()[Skills.STRENGTH].decreaseLevel(random
                            .inclusive(1, 10));
                    Skills.refresh(victim, Skills.STRENGTH);
                    victim.graphic(new Graphic(400));
                }
                else if (Combat.isFullKarils(builder.getCharacter()) &&
                        victim.getSkills()[Skills.AGILITY].getLevel() >=
                                victim.getSkills()[Skills.AGILITY]
                                        .getRealLevel()) {
                    victim.graphic(new Graphic(401));
                    victim.getSkills()[Skills.AGILITY].decreaseLevel(random
                            .inclusive(1, 10));
    ...
    private void handlePrayerEffects() {
        if (builder.getVictim().getType() == NodeType.PLAYER && data.getHits
                ().length != 0) {
            Player victim = (Player) builder.getVictim();

            if (CombatPrayer.isActivated(victim, CombatPrayer.REDEMPTION) &&
                    victim.getSkills()[Skills.HITPOINTS].getLevel() <=
                            (victim.getSkills()[Skills.HITPOINTS]
                                    .getRealLevel() / 10)) {
                int heal = (int) (victim.getSkills()[Skills.HITPOINTS]
                        .getRealLevel() * Combat.REDEMPTION_PRAYER_HEAL);
                victim.getSkills()[Skills.HITPOINTS].increaseLevel(random
                        .inclusive(1, heal));
                victim.graphic(new Graphic(436));
                victim.getSkills()[Skills.PRAYER].setLevel(0, true);
                victim.getEncoder().sendMessage("You've run out of prayer " +
                        "points!");
            ...
            if (builder.getCharacter().getType() == NodeType.PLAYER) {
                if (CombatPrayer.isActivated(victim, CombatPrayer
                        .RETRIBUTION) && victim.getSkills()[Skills.HITPOINTS]
                        .getLevel() < 1) {
                    victim.graphic(new Graphic(437));

                    if (Location.inWilderness(victim) || MinigameHandler
                            .contains(victim) && !Location.inMultiCombat
                            (victim)) {
                        if (builder.getCharacter().getPosition()
                                .withinDistance(victim.getPosition(), Combat
                                        .RETRIBUTION_RADIUS)) {
                            builder.getCharacter().damage(new Hit(random
                                    .inclusive(Combat
                                            .MAXIMUM_RETRIBUTION_DAMAGE)));
                        }
                    }
                    else if (Location.inWilderness(victim) || MinigameHandler
                            .contains(victim) && Location.inMultiCombat
                            (victim)) {
                        for (Player player : victim.getLocalPlayers()) {
                            ...
                            if (!player.equals(victim) && player.getPosition
                                ...
                                player.damage(new Hit(random.inclusive(Combat
                                        .MAXIMUM_RETRIBUTION_DAMAGE)));
                            }
                        }
                    }
                }
                if (CombatPrayer.isActivated((Player) builder.getCharacter(),
                        CombatPrayer.SMITE)) {
                    victim.getSkills()[Skills.PRAYER].decreaseLevel(counter /
                            4);
                    Skills.refresh(victim, Skills.PRAYER);
                }
            }
    ...
    private void handleExperience() {
        if (data.getExperience().length == 0 && data.getType() != CombatType
                .MAGIC) {
            return;
        }
        if (builder.getCharacter().getType() == NodeType.PLAYER) {
            ...
            if (data.getType() == CombatType.MAGIC) {
                exp = (counter * 4d) + builder.getCharacter()
